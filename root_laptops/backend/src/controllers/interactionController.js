...
async function addToCart(userId, watchid, quantity, price) {
  let cart = await Cart.findOne({ user_id: userId });
  if (!cart) {
    cart = await Cart.create({ user_id: userId });
  }
  const cartItem = await CartItem.create({ cart_id: cart._id, watch_id: watchid, quantity, price });
...
async function createOrder(userId, total_amount, shipping_address, payment_method, items) {
  ...
  const orderItems = await OrderItem.insertMany(
    items.map(item => ({ order_id: order._id, watch_id: item.watch_id, quantity: item.quantity, price: item.price }))
  );
