...
export function ProductGallery({ images, productTitle }: ProductGalleryProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 lg:sticky top-32 self-start">
      <div className="relative w-full h-[500px] bg-muted rounded-lg flex items-center justify-center p-8">
        {activeImg && (
          <Image
            src={activeImg}
            layout="fill"
      ...
      <div className="flex gap-3 justify-center mt-2 flex-wrap">
        {images.map((img, index) => (
          <button
            ...
            className={`h-16 w-16 rounded-md border-2 p-1 ${activeThumbIndex === index ? 'border-primary' : 'border-transparent'}`}
